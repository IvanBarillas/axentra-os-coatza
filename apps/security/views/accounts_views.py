...
from apps.security.models import User, UserProfile, Sede, Dependencia, AreaOperativa
from apps.security.selectors import FuncionarioSelectors
from apps.security.services import FuncionarioService
from apps.security.forms import StaffUserCreationForm, StaffUserProfileForm, StaffUserChangeForm, StaffUserProfileChangeForm, AdminPasswordChangeForm

@login_required
@axentra_gate_enforcer(AppIdentifier.SECURITY, required_fine_permission='can_view_list')
...
@login_required
@axentra_gate_enforcer(AppIdentifier.SECURITY, required_fine_permission='can_edit_user')
def funcionario_create_view(request):
    """Orquestador transaccional de altas de servidores públicos."""
    if request.method == 'POST':
        ...
        if form.is_valid() and profile_form.is_valid():
            ...
            if exito and usuario:
                ...
                return redirect('security:funcionario_list')
            if errores:
...
def funcionario_editar_view(request, pk: uuid.UUID):
    """Modificación síncrona y atómica de expedientes de adscripción."""
    ...
    return render(request, 'accounts/forms/funcionario_update_form.html', {
        'form_user': form_user, 'form_profile': form_profile, 'funcionario': FuncionarioSelectors.obtener_por_id(pk)
    })

@login_required
@axentra_gate_enforcer(AppIdentifier.SECURITY, required_fine_permission='can_change_password')
def funcionario_cambiar_password_view(request, pk: uuid.UUID):
    """Cifrado administrativo forzado de llaves de acceso."""
...
@require_POST
@login_required
@axentra_gate_enforcer(AppIdentifier.SECURITY, required_fine_permission='can_delete_user')
def funcionario_soft_delete_view(request, pk: uuid.UUID):
    """Baja lógica perimetral que congela la identidad del usuario."""
    if str(pk) == str(request.user.id):
        messages.error(request, "Operación denegada: No puede suspender su propia cuenta.")
        return redirect('security:funcionario_list')
