...
class AssetReassignmentForm(InventoryForm):
    asset_id=UUIDChoiceField(label="Bien patrimonial"); user_id=UUIDChoiceField(label="Nuevo resguardatario")
    ...
    def to_dto(self): d=self.require_cleaned_data(); return ReassignAssetDTO(d["asset_id"],d["user_id"],d["reason"],occurred_at=d.get("occurred_at"))


class AssetLocationChangeForm(InventoryForm):
    asset_id=UUIDChoiceField(label="Bien patrimonial"); site_id=UUIDChoiceField(label="Sede destino"); area_id=UUIDChoiceField(label="Área operativa destino")
    ...
    def clean(self):
        d=super().clean()
        if d.get("site_id") and not d.get("area_id"):
            self.add_error("area_id", "Seleccione el área correspondiente a la sede destino.")
        return d
    def to_dto(self):
        ...
        return ChangeAssetLocationDTO(d["asset_id"],d["reason"],area_id=d.get("area_id"),site_id=d.get("site_id"),geolocation=geo,occurred_at=d.get("occurred_at"))
