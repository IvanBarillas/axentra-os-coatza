...
LOAN_WORKFLOW = WorkflowDefinition(
    ...
    actors=(actor("origin", "Dependencia propietaria"), actor("destination", "Dependencia receptora")),
    steps=(
        ...
        WorkflowStep("receipt", "PENDING_RECEIPT", "Subir acuse firmado", "origin"),
        WorkflowStep("confirmation", "PENDING_CONFIRMATION", "Confirmar acuse", "destination"),
        WorkflowStep("active", "ACTIVE", "Usar temporalmente", "destination"),
        WorkflowStep("return", "RETURN_REQUESTED", "Subir acuse de devolución", "destination"),
        WorkflowStep("closed", "RETURNED", "Confirmar devolución y cerrar", "origin", terminal=True),
    ...
    transitions=(
        ...
        WorkflowTransition("reception", "receipt", "Aceptar", "success", permission="can_authorize_loans"),
        WorkflowTransition("reception", "rejected", "Rechazar", "danger", permission="can_authorize_loans"),
        WorkflowTransition("receipt", "confirmation", "Integrar acuse", permission="can_request_loans"),
        WorkflowTransition("confirmation", "active", "Confirmar entrega", "success", permission="can_authorize_loans"),
        WorkflowTransition("active", "return", "Devolver", permission="can_authorize_loans"),
        WorkflowTransition("return", "closed", "Confirmar recepción", "success", permission="can_request_loans"),
    ),
    primary_path=("draft", "reception", "receipt", "confirmation", "active", "return", "closed"),
    notes=("El bien sigue perteneciendo a la dependencia de origen.", "Cada acuse lo confirma la dependencia contraparte.", "Patrimonio sólo interviene en préstamos externos o excepciones."),
)
